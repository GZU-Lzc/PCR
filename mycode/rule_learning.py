...
class Rule_Learner(object):
    ...
    def create_rule(self, walk, type):
        """
    ...
    def estimate_confidence(self, rule, num_samples=500):
        ...
        all_rules = []
        rel_edges = self.edges[rule["head_rel"]]
        for edge in rel_edges:
            all_rules.append(edge.tolist())
        all_rules.sort()
        unique_rules = list(x for x, _ in itertools.groupby(all_rules))
        rule_support = len(unique_rules)

        confidence, up, rule_body_support = 0, 0, 0
        if body_support and rule_support:
            rule_body_support = self.calculate_rule_support(unique_bodies, rule)
            confidence = round(rule_body_support / body_support, 6)
            up = round(confidence * num_samples / rule_support, 6)

        return confidence, up, rule_support, body_support, rule_body_support
    ...
    def calculate_rule_support(self, unique_bodies, rule):
        """
        ...
        rule_support = 0
        head_rel_edges = self.edges[rule['head_rel']]
        for body in unique_bodies:
            if rule['type'] == 'Head':
                mask = (
                    (head_rel_edges[:, 0] == body[0])
                    * (head_rel_edges[:, 2] == body[2])
                    * (head_rel_edges[:, 3] > body[-2] - globals.delta)
                )
            elif rule['type'] == 'Middle':
                mask = (
                    (head_rel_edges[:, 0] == body[2])
                    * (head_rel_edges[:, 2] == body[-3])
                    * (head_rel_edges[:, 3] > body[-2] - globals.delta)
                )
            else:
                mask = (
                        (head_rel_edges[:, 0] == body[-3])
    ...
    def sort_rules_dict(self):
        ...
        for rel in self.rules_dict:
            self.rules_dict[rel] = sorted(
                self.rules_dict[rel], key=lambda x: (x["conf"], x["rule_body_supp"]), reverse=True
            )
...
def verbalize_rule(rule, id2relation):
    ...
    rule_str = "{0:8.6f}  {1:4}  {2:4}  {3}(X{4},X{5},T{6}) <- "
    if rule['type'] == 'Head':
        sub_idx = 0
        obj_idx = [
            idx
            for idx in range(len(var_constraints))
            if 1 in var_constraints[idx]
        ][0]
    elif rule['type'] == 'Middle':
        sub_idx = [
            idx
        ...
        obj_idx = [
            idx
            for idx in range(len(var_constraints))
            if 2 in var_constraints[idx]
        ][0]
    else:
        sub_idx = [
            idx
            for idx in range(len(var_constraints))
            if len(rule["body_rels"]) - 1 in var_constraints[idx]
        ][0]
        obj_idx = [
            idx
            for idx in range(len(var_constraints))
