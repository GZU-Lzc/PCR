    ...
    for k in relations_idx:
        ...
        for length in rule_lengths:
            ...
            for _ in range(num_walks):
                walk_successful, walk = temporal_walk.sample_walk(length + 1, rel, 'Head')
                if walk_successful:
                    rl.create_rule(walk, 'Head')
            if length > 2:
                for _ in range(num_walks):
                    walk_successful, walk = temporal_walk.sample_walk(length + 1, rel, 'Middle')
                    if walk_successful:
                        rl.create_rule(walk, 'Middle')
            for _ in range(num_walks):
                walk_successful, walk = temporal_walk.sample_walk(length + 1, rel, 'Tail')
                if walk_successful:
                    rl.create_rule(walk, 'Tail')
            it_end = time.time()
