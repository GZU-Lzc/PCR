...
def filter_rules(rules_dict, min_conf, min_rule_body_supp, rule_lengths):
    """
    ...
    for k in rules_dict:
        ...
        for rule in rules_dict[k]:
            cond = (
                (rule["conf"] >= min_conf)
                and (rule["rule_body_supp"] >= min_rule_body_supp)
...
def match_body_relations(rule, edges, test_query_sub):
    ...
    try:
        rel_edges = edges[rels[start]]
        mask = rel_edges[:, 0] == test_query_sub
        new_edges = rel_edges[mask]
        walk_edges = [
            np.hstack((new_edges[:, 0:1], new_edges[:, 2:4]))
        ]  # [sub, obj, ts]
        cur_targets = np.array(list(set(walk_edges[0][:, flag1])))

        if rule['type'] == 'Middle':
            try:
                rel_edges = edges[rels[0]]
                mask = np.any(rel_edges[:, flag2] == cur_targets[:, None], axis=0)
                new_edges = rel_edges[mask]
                walk_edges.insert(
                    0, np.hstack((new_edges[:, 0:1], new_edges[:, 2:4]))
                )  # [sub, obj, ts]
            ...
            try:
                ...
                mask = np.any(rel_edges[:, flag1] == cur_targets[:, None], axis=0)
                new_edges = rel_edges[mask]
                walk_edges.append(
                    np.hstack((new_edges[:, 0:1], new_edges[:, 2:4]))
                )  # [sub, obj, ts]
            ...
            for i in range(1, len(rels)):
                ...
                try:
                    ...
                    cur_targets = np.array(list(set(walk_edges[i][:, flag1])))
                except KeyError:
                    walk_edges.append([])
                    break
...
def get_walks(rule, walk_edges):
    ...
    for i in range(1, len(df_edges)):
        ...
        if (rule['type'] == 'Head' and i != 1 and not rule["var_constraints"]) or (rule['type'] == 'Middle' and i != 2 and not rule["var_constraints"]) or (rule['type'] == 'Tail' and not rule["var_constraints"]):
...
def save_candidates(
    ...
    for k in all_candidates:
        all_candidates[k] = {cand: v for cand, v in all_candidates[k].items()}
    ...
    try:
        ...
        # 合并已有数据和新数据
    merged_data = {**existing_data, **all_candidates}

    # 将合并后的数据写回文件
    with open(file_path, "w", encoding="utf-8") as fout:
        json.dump(merged_data, fout)
        # json.dump(merged_data, fout, cls=MpmathEncoder)
